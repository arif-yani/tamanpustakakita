...
class sgeede_facebook_comment(models.Model):
    ...
    tag_id = fields.Many2one('product.tag',string="Code")
    ...
    def get_all_comments(self, post_id, page_id):
        ...
        if page_id and post_id:
            if len(post_id.attachment_ids) > 1:
                data = []
                for attch in post_id.attachment_ids:
                        api_get_comment = api_url+attch.id_attachment+"/comments?access_token="+page_id.page_token
                        headers = {
                                'Content-Type':'application/json'
                            }
                        response_api = requests.get(api_get_comment, headers=headers)
                        comment_json = response_api.json()
                        for item in comment_json['data']:
                            item['id_attachment'] = attch.id_attachment
