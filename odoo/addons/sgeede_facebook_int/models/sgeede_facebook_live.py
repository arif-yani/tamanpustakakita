...
class sgeede_facebook_live(models.Model):
    ...
    duration = fields.Float('Duration Live', compute="_compute_duration_live", tracking=True)
    start_live = fields.Datetime('Start Live', tracking=True)
    end_live = fields.Datetime('End Live')
    live_date = fields.Date('Live Date')
    facebook_live_id = fields.Char('Live ID', required=True, tracking=True)
    line_ids = fields.One2many('sgeede.facebook.product', 'live_id',string="Active Product", tracking=True)
    comment_ids = fields.One2many('sgeede.facebook.comment', 'live_id', string="Comment")
    is_active = fields.Boolean('Live', default=False)
    state = fields.Selection([('draft','Draft'), ('live','On Live'), ('done','Done'), ('cancel','Cancel')], default="draft", tracking=True)
    note = fields.Html('Note')
    partner_id = fields.Many2one('res.partner', 'Host', required=True, tracking=True)
    page_id = fields.Many2one('sgeede.facebook.page', 'Page', required=True, tracking=True)
...
class sgeede_facebook_product(models.Model):
    ...
    product_id = fields.Many2one('product.product', string="Active Product", tracking=True)
    product_template_id = fields.Many2one('product.template', string="Active Product")
    product_qty = fields.Float(string="Stock", tracking=True)
