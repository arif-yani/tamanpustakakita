...
class ResPartner(models.Model):
    ...
    def _compute_total_titipan(self):
        for line in self:
            ...
            if get_paket_id:
                line.total_titipan =  sum([paket.weight for paket in get_paket_id if paket.is_titipan and not paket.is_hide_paket])
            else:
    ...
    def _compute_invoice_link(self):
        for line in self:
            ...
            #     'partner_ids': invoice_obj.partner_id.ids,
            #     'res_id': invoice_obj.id,
            #     'res_model': 'account.move'
            # })
            if invoice_obj:
                line.invoice_link_download = invoice_obj.invoice_link_download
            else:
