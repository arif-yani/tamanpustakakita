...
class MyfutureCustomerPortal(PortalAccount):
    ...
    def portal_my_invoice_detail(self, invoice_id, access_token=None, report_type=None, download=False, **kw):
        ...
        if report_type == 'pdf' and download and invoice_sudo.state == 'posted':
            ...
            # attachments = request.env['ir.attachment'].browse(self._check_availabel_attachment(invoice_sudo))
            # attachments.res_model = 'ir.attachment'
            # attachments.res_id = attachments.id
            ...
            # print("masuk1",template)
            return self._show_report(model=invoice_sudo, report_type=report_type, report_ref='my_future_custom.action_report_invoice', download=download)


        elif report_type in ('html', 'pdf', 'text'):
            # print("masuk2",invoice_sudo, report_type, download)
            return self._show_report(model=invoice_sudo, report_type=report_type, report_ref='my_future_custom.action_report_invoice', download=download)

        values = self._invoice_get_page_view_values(invoice_sudo, access_token, **kw)
        return request.render("account.portal_invoice_page", values)
    
    def _check_availabel_attachment(self, move_id):
        check_attachment = request.env['ir.attachment'].sudo().search([('res_id','=',move_id.id),('res_model','=','account.move')], limit=1)
        if check_attachment:
            ...
            attachment = self.env['ir.attachment'].create({
                'type': 'url',
                'name': move_id.name,
                'url': '/report/pdf/my_future_custom.report_invoice_myfuture/%s'%move_id.id,
