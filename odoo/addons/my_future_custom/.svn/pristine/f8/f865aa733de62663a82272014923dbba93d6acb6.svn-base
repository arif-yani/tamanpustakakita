...
class my_future_facebook_keep_order(models.Model):
    ...
    def action_send_message_waiting_wa(self):
        keep_ids = self.search([('id','in',self._context['active_ids']),('state','=','waiting_wa')])
        print("check_id", keep_ids.partner_id)
        if keep_ids.partner_id and keep_ids.comment_id.post_id.page_id[0]:
            self.send_need_mobile(keep_ids.partner_id, keep_ids.comment_id.post_id.page_id[0])


    def send_need_mobile(self, partner_ids, page_id):
        for partner_id in partner_ids: 
            api_url = "https://graph.facebook.com/v19.0/"
            page_id.cron_generate_token_data()
            api_post_comment = api_url+page_id.page_id+"/messages?access_token="+page_id.page_token
            messager_obj =  self.env['whatsapp.template'].search([('template_name','=','messanger_request_number_phone_from_user')], limit=1)
            send_message_obj = self.env['whatsapp.composer'].create({
                    'wa_template_id': messager_obj.id,
                    'phone': partner_id.mobile,
                    'res_ids': partner_id.ids,
                    'res_model': 'mres.partner',
                })
            body = send_message_obj._get_html_preview_whatsapp(rec=partner_id)
            ...
            reply_message = {
                "recipient":{
                    "id": partner_id.facebook_account_id
                },
            ...
            if response.status_code == 200:
                partner_id.can_interaction = True
            else:
                print("failed to send", partner_id.name, response.json())
                partner_id.can_interaction = False
