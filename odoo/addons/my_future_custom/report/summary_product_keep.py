	...
	def generate_xlsx_report(self, workbook, data, orders):
		# for doc in orders:
			...
			for summary in orders.summary_product_keep():
				row += 1
				sheet.write(row,0, summary['product'].default_code if summary['product'].default_code else "", body_table_format)
				sheet.write(row,1, summary['product'].name, body_table_format)
				sheet.write(row,2, summary['size'].name if summary['size'] else "", body_table_format)
